...
export default function Home({ user }) {
  ...
  const eta = useMemo(() => {
    const anni = drawings
      .map(d => d.child_age_months ? Math.floor(d.child_age_months / 12) : null)
      .filter(Boolean)
    return [...new Set(anni)].sort((a, b) => a - b)
  }, [drawings])

  const drawingsFiltrati = useMemo(() => {
    let lista = [...drawings]
    if (filtri.categoria) lista = lista.filter(d => d.category === filtri.categoria)
    if (filtri.eta) lista = lista.filter(d =>
      d.child_age_months && Math.floor(d.child_age_months / 12) === parseInt(filtri.eta)
    )
  ...
  const filtriAttivi = filtri.categoria || filtri.eta || filtri.ordinamento !== 'recenti'

  const resetFiltri = () => setFiltri({ categoria: '', eta: '', ordinamento: 'recenti' })

  const fetchTutteLeStorie = async () => {
        ...
        <div style={{ padding: '20px 16px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.85rem', color: '#999', margin: 0 }}>
              {drawingsFiltrati.length} disegn{drawingsFiltrati.length === 1 ? 'o' : 'i'}
          ...
          <div style={{
            ...
            {drawingsFiltrati.map(drawing => (
              <DrawingCard
                ...
                onElimina={(id) => setConfermaElimina(id)}
              />
            ))}
          </div>
        )}
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              {filtriAttivi && (
                <button onClick={resetFiltri} style={{
                  background: 'none', border: 'none', cursor: 'pointer',
                  fontFamily: 'Inter, sans-serif', fontWeight: 600,
                  fontSize: '0.85rem', color: '#FF7F6A'
                }}>
                  Reset filtri
                </button>
              )}
            ...
            {categorie.length > 0 && (
              <div style={{ marginBottom: '24px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  <button onClick={() => setFiltri(f => ({ ...f, categoria: '' }))}
                    style={chipStyle(!filtri.categoria)}>
                    Tutte
                  </button>
                  {categorie.map(cat => (
                    <button key={cat} onClick={() => setFiltri(f => ({ ...f, categoria: cat }))}
                      style={chipStyle(filtri.categoria === cat)}>
                      {cat}
                    </button>
                  ))}
                </div>
            ...
            {eta.length > 0 && (
              <div style={{ marginBottom: '24px' }}>
                <p style={labelFiltroStyle}>Età</p>
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  <button
                    onClick={() => setFiltri(f => ({ ...f, eta: '' }))}
                    style={chipStyle(!filtri.eta)}
                  >
                    Tutte
                  </button>
                  {eta.map(e => (
                    <button
                      key={e}
                      onClick={() => setFiltri(f => ({ ...f, eta: String(e) }))}
                      style={chipStyle(filtri.eta === String(e))}
                    >
                      {e} anni
                    </button>
                  ))}
      ...
      {libroAperto && (
        <div style={{
          ...
          >
            ...
            <h3 style={{
              fontFamily: 'Outfit, sans-serif', fontWeight: 800,
              fontSize: '1.3rem', color: '#2D2D2D', margin: '0 0 4px 0'
            }}>
            ...
            <p style={{
              ...
              Seleziona 2 o più storie per unirle in una nuova avventura
            </p>

            {tutteLeStorie.length === 0 ? (
              <p style={{
                textAlign: 'center', color: '#999',
                fontFamily: 'Inter, sans-serif', padding: '20px'
              }}>
                Nessuna storia ancora. Genera le prime storie dai disegni! ✨
              </p>
            ) : (
              <>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
                  {tutteLeStorie.map(s => {
                    const selezionata = storieSelezionate.includes(s.id)
                    return (
                      <div
                        key={s.id}
                        onClick={() => setStorieSelezionate(prev =>
                          selezionata ? prev.filter(sid => sid !== s.id) : [...prev, s.id]
                        )}
                        style={{
                          display: 'flex', alignItems: 'center', gap: '12px',
                          padding: '12px 14px', borderRadius: '16px',
                          border: `2px solid ${selezionata ? '#A084E8' : '#e8e4df'}`,
                          backgroundColor: selezionata ? '#A084E808' : 'white',
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{
                            fontFamily: 'Outfit, sans-serif', fontWeight: 700,
                            fontSize: '0.88rem', color: '#2D2D2D', margin: '0 0 2px 0',
                          ...
                          <p style={{
                            ...
                            {s.tipo === 'breve' ? '✨ Breve' : '📖 Favola'} · {new Date(s.created_at).toLocaleDateString('it-IT', { day: '2-digit', month: 'short' })}
                          </p>
                        </div>
                        <div style={{
                          width: '22px', height: '22px', borderRadius: '50%',
                          border: `2px solid ${selezionata ? '#A084E8' : '#ddd'}`,
                          backgroundColor: selezionata ? '#A084E8' : 'white',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                          flexShrink: 0
                        }}>
                          {selezionata && <span style={{ color: 'white', fontSize: '0.75rem' }}>✓</span>}
                        </div>
                      </div>
                    )
                  })}
                ...
                {storieSelezionate.length >= 2 && !storiaUnita && (
                  <div style={{ marginBottom: '16px' }}>
                    <textarea
                      ...
                      onChange={e => setIndicazioniUnione(e.target.value)}
                      rows={2}
                      style={{
                ...
                {storieSelezionate.length >= 2 && !storiaUnita && (
                  <button
                    onClick={handleUnisciStorie}
                    disabled={loadingUnione}
                    style={{
                      width: '100%', padding: '15px',
                      background: 'linear-gradient(135deg, #FF7F6A, #A084E8)',
                      border: 'none', borderRadius: '50px', cursor: 'pointer',
                      fontFamily: 'Outfit, sans-serif', fontWeight: 700,
                      fontSize: '1rem', color: 'white', marginBottom: '12px',
                  ...
                  >
                    {loadingUnione ? (
                      ...
                      <>🪄 Unisci {storieSelezionate.length} storie</>
                    )}
                  </button>
                )}
                  ...
                  <div style={{ marginBottom: '16px' }}>
                    <div style={{
                      ...
                      <p style={{
                        ...
                        color: '#2D2D2D', lineHeight: 1.8, margin: 0,
                        whiteSpace: 'pre-wrap'
                      }}>
                        {storiaUnita}
                      </p>
                    ...
                    <div style={{ display: 'flex', gap: '10px' }}>
                      <button
                        onClick={() => navigator.clipboard.writeText(storiaUnita)}
                        style={{
                          flex: 1, padding: '12px',
                          background: 'none', border: '2px solid #A084E8',
                          borderRadius: '50px', cursor: 'pointer',
                          fontFamily: 'Inter, sans-serif', fontWeight: 600,
                      ...
                      <button
                        onClick={() => { setStoriaUnita(null); setStorieSelezionate([]) }}
                        style={{
                          flex: 1, padding: '12px',
                          background: '#f5f3f0', border: 'none',
                          borderRadius: '50px', cursor: 'pointer',
                          fontFamily: 'Inter, sans-serif', fontWeight: 600,
                          fontSize: '0.85rem', color: '#666'
                        }}
